<template>
  <div class="container-fluid px-4" :class="{ 'hidden': currentComponent === 'Sales' }">
    ...
    <div v-else>
      <div class="card mt-4 mb-3">
        <div class="card-header border-bottom-0 p-4">
          ...
          <span class="text-muted h5"> / Sales-List</span>
        </div>
      </div>
      <div class="card mb-4 mt-3 tt">
        <div class="card-header d-flex justify-content-between">
          ...
          <div class="link_button">
            <button class="btn btn-primary" @click="currentComponent = 'Sales'">Add Sales</button>
          </div>
        </div>
        <div class="card-body">
...
export default {
  ...
  methods: {
    async fetchCompanyWistPurchase() {
      this.loading = true;
      await axios.get(`/api/sales_products/${this.company_id}`)
        .then((res) => {
          this.sales = res.data
        })
        .catch((error) => {
